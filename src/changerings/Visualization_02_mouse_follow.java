...
public class Visualization_02_mouse_follow extends PApplet {
	...
	void drawMusic2() {
		...
		/* if (v%RASTRO == 0) {
		 ...
		 // translate(width/2, height/20);  //height/1.1);   // MORE INTERESTING WHEN IMAGE IS OFF CENTER
		  ...
		  if (bands[0] > 1) {
		    ...
		    stroke(255, 255, 0);
		    //noFill();
		    fill(255*band_m0, 180, 255); //130);   //  BLUE    //255, 200);  // AQUA //, 100);
		    ellipse(pmouseX, pmouseY, bands[0]*4, bands[0]*4);    //0, 0, bands[0]*10, bands[0]*10);
		  } 
		  if (bands[1] > 20) {
		    strokeWeight(15);
		    stroke(0);   //(255, 197, 80);   //, 125);
		    noFill();
		    rotate(PI/random(5));
		    triangle(-10*bands[1], -10*bands[1], mouseX, 10*bands[1], 10*bands[1], -10*bands[1]);
		...
		/*  if (bands[3] > 5) {
		    strokeWeight(band_m3*20);   //50);   //*100);    SPIRAL BANDS
		    stroke (0);  //(255*band_m3, 255, 255);   //255, 255, 255);  //*band_m3, 255, 255);  //255, 128);   //, 100);
		    ...
		    spiral(pmouseX, pmouseY, (int)(band_m3*2*width));  //50*band_m3, bands[3], (int)(band_m3*2*width));
		    strokeWeight(0);
		  } */
		  if (bands[4] > 1) {
		    strokeWeight(3);
		    stroke(255, 0, 0);
		    fill(c1);  //255, 129, 247);   //, 180);   // ROSE-COLORED SQUARES // (255*band_m4, 255, 255, 200);
		    rect(random(width/2), random(height/2), band_m4*280, band_m4*280);  //random(-width/2, width/2), random(-height/2, height/2), band_m4*280, band_m4*280);
		  }
		  if (bands[5] > 0.5) {
		    rectMode(CENTER);
		    strokeWeight(2);
		    stroke(0);
		    fill(254, 255, 9);   // YELLOW SQUARES  //255*band_m5); 
		    rect(width/2, height/2, band_m5*250, band_m5*250);  //random(-width/2, width/2), random(-height/2, height/2), band_m5*250, band_m5*250);
		   rectMode(CORNER);
		  } 		
	}
